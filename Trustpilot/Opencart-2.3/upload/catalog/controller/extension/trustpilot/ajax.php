            ...
            if (($this->request->server['REQUEST_METHOD'] == 'POST')) {
                ...
                if ($post['action'] == 'handle_past_orders') {
                    ...
                    if (array_key_exists('sync', $post)) {
                        $this->model_extension_trustpilot_past_orders->syncPastOrders($post['sync']);
                        $output = $this->model_extension_trustpilot_past_orders->getPastOrdersInfo();
                        $this->createResponse($output, 'plugin');
                        return;
                    }
                    if (array_key_exists('resync', $post)) {
                        $this->model_extension_trustpilot_past_orders->resyncPastOrders();
                        $output = $this->model_extension_trustpilot_past_orders->getPastOrdersInfo();
                        $this->createResponse($output, 'plugin');
                        return;
                    }
